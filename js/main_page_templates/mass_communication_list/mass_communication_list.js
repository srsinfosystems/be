...
var mass_communication_list = {
	start:function(){
		...
		var total_params = {
			...
			getTranslationsDataArray:['alert message','Dashboard','Step','of','Cancel','Mass communication','Communications','Select recipients','Select distribution properties','Compose message','Summary','Customer groups','Custom groups','Edit','Group name','Customers','Action','Save','Search customer','Customer name','Customer number','Next','Previous','Add group','No group found','$customer_group_err','$custom_group_err','Continue','Next','Please select custom group','Please select group with customer','Back','Distribution properties','Both','Send email','Send SMS','Recipients','Default customer contact','Address listed on the customer card','All contact persons','Distribution','SMS sender','Message details','Subject','Email text','Delete','$del_cust_grp','$del_cust_grp_suc','SMS text','Custom text','Placeholders','Add','Please check the following fields','File name','$add_files','$attach_del','Confirmation','Messages sent','$mass_is_distr','Description','Quantity','$sms_sent','Emails sent','Customers reached','$recp_count','Send another message','Continue1','$email','SMS','Attachments','Send','No preview available','SMS text','Custom lists','Queries','Customer',
				...
				'Status','Participant','Exempt','Specialty work','Schedule Date','Schedule status','Completed','In progress','Voluntary work','Balance','Marina','Dock','Slip','Meter','$customerh','Run query','Registration date','Customer name','City','Country','Zip','Save and continue','Please select query','Save Query','Name','Save as','A query','A custom list','Empty query','Type','Select','$qry_del','Warning','Delete','Success','No record found','De select','Create query','$meterh'
				],
		};
	
		var params = $.extend({}, doAjax_params_default);
		params['url'] = APISERVER+'/api/Commons/mass_comm_all.json';
		...
		params['successCallbackFunction'] = function (complet_data){
			...
			if(complet_data.response.status == 'success'){
				...
				mcl_td= complet_data.response.response.translationsData;
				mass_communication_list.createHtml(complet_data.response.response);
			}
	...
	createHtml:function(){
		mcl_dt.date_format_partner = date_format_partner;
		mcl_dt.translationsData.dashboardurl = base_url+'dashboard/index';
		mcl_dt.translationsData.loaderurl = host_url+'app/webroot/img/loading.gif';


		var template = document.getElementById('mass_communication_list_template').innerHTML;
		var compiledRendered = Template7(template, mcl_dt);
		document.getElementById('content').innerHTML = compiledRendered;
		hideProcessingImage();		
		mass_communication_list.bindEvents();
	},
	bindEvents:function(){
		mass_communication_list.bindQuery();
		mass_communication_list.renderSmsSender();
		mass_communication_list.generatePlaceholdersList()
            	...
            	if(index==1){
        		 	if($('#masscust').val()=='custom_groups'){
	            		if(customer_custom_groups.length==0){
		            		showAlertMessage(mcl_td.Pleaseselectcustomgroup,'error',mcl_td.alertmessage);
		            		return false;
		            	}
	            	}
	            	else if($('#masscust').val()=='customer_groups'){
		            	if(customer_groups.length==0){
		            		showAlertMessage(mcl_td.Pleaseselectgroupwithcustomer,'error',mcl_td.alertmessage);
		            		return false;
		            	}
		            	else{
		            		var cust_exists = 0;
	            		...
	            		if(customer_queries.length==0){
		            		showAlertMessage(mcl_td.Pleaseselectquery,'error',mcl_td.alertmessage);
		            		return false;
		            	}
	            	...
	            	var v = $('#TriggerAction').val();
	            	var err = '';
	            	var sms = $('#TriggerCustomTextSms').val();
	            	var email_text = $('#custom_text').val();
	            	var email_sub = $('#TriggerCustomSubject').val();
	            	if(v=='send_sms'){
                		if(checkNull(sms)==''){
                			err += gbl_trnsdata.SMStext+'<br/>';
                		}
                	}
                	else if(v=='send_email'){
                		if(checkNull(email_sub)==''){
                			err += gbl_trnsdata.Subject+'<br/>';
                		}
                		if(checkNull(email_text)==''){
                			err += gbl_trnsdata.Emailtext+'<br/>';
                		}
                	}
                	else if(v=='both'){
                		if(checkNull(email_sub)==''){
                	...
                	if(err!=''){
                		showAlertMessage(mcl_td.Pleasecheckthefollowingfields+'<br/>'+err,'error',mcl_td.alertmessage);
		            	return false;
                	}
	            }
            ...
            onNext: function (tab, navigation, index) {
                ...
                for (var i = 0; i < index; i++) {
              	...
              	if(current == 1){
              		...
              		var v = $('#TriggerAction').val();
                	$('.email_text_wrapper').addClass('hide');
                	$('.sms_text_wrapper').addClass('hide');
                	if(v=='send_sms'){
                		$('.sms_text_wrapper').removeClass('hide');
                	}
                	else if(v=='send_email'){
                		$('.email_text_wrapper').removeClass('hide');	
                	}
                	else if(v=='both'){
                		$('.sms_text_wrapper').removeClass('hide');
                		$('.email_text_wrapper').removeClass('hide');
         	 	...
         	 	if(current == 1 ){
                	mass_communication_list.checkNextButton();
                }
                else{
		...
		$('#form_wizard_1').find('.button-send').click(function(){
			var v = $('#TriggerAction').val();
	    	var err = '';
	    	var sms = $('#TriggerCustomTextSms').val();
	    	var email_text = $('#custom_text').val();
	    	...
	    	mass_communication_list.sendEmail();
		});
		...
		$('#TriggerAction,#TriggerTo').select2({
			minimumResultsForSearch:-1
		});

		$('#TriggerAction').change(function(){
			var v = $(this).val();
			if(v=='both' || v=='send_sms'){
				$('.sms_sender_wrapper').removeClass('hide');
			}
			else{
				$('.sms_sender_wrapper').addClass('hide');
			}
		});

		wsy = $('#custom_text').wysihtml5();

		$('#PartnerEmailPlaceholders').select2();

		$("#TriggerCnt").val('160 / 1');
		$("#TriggerCustomTextSms").keypress(function(){
			var count_elem 	= get_multipart_count($("#TriggerCustomTextSms").val());
			var counter 	= multipart_count($("#TriggerCustomTextSms").val());
			if(counter == '1'){
				var value 		= 160 - count_elem;
			}else {
				var value 		= (counter*153) - count_elem;
			}
			$("#TriggerCnt").val(value + ' / '+ counter);
			$("#counter").val(counter);
		});		

		var total_params = 'token='+token+'&language='+language+'&lang='+lang+'&partner_id='+partner_id+'&admin_id='+admin_id+'';
		$('#filess').fileupload({
		    acceptFileTypes: /(\.|\/)(gif|jpe?g|png|bmp|txt|doc|docx|xlsx|xls|pdf|zip|csv)$/i,
		         	...
		         	$('tr.attachments').hide();
							$.each(result.response.response.files, function (index, file) { 
								...
								if(file.error != undefined || file.error != null){
									...
									var err = '';
									var addHtml = '';
									err = mcl_td.Nopreviewavailable;
										fancy_count++;
									if((extension == 'jpg') || (extension == 'png') || (extension == 'gif') || (extension == 'jpeg')){
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
										
												addHtml += '<div class="first">';
													if(file.thumbnail_url != undefined){
														addHtml += '<img class="attach_img" src="'+file.thumbnail_url+'">';
													}
													else{
														addHtml += '<img class="attach_img" src="'+file.url+'">';
													}
													addHtml += '<div class="triangles"><div id="" class="triangles_img" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second">';
													
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_img.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name attach_name_img">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size attach_size_img">' + size  + '</div>';
														addHtml += '</div>';
												...
												addHtml += '</div>';
											addHtml += '</a>';
											addHtml += '<button class="hover_remove btn mini " onclick="mass_communication_list.deleteFile(\''+delete_url+'\',this)">';
												addHtml += '<i class="icon-remove"></i>';
											addHtml += '</button>';
												var b = JSON.stringify(file);
												b = b.replace(/"/g , "'");
												addHtml += '<input type="hidden" class="uploaded_file" value="'+b+'">';
										addHtml += '</div>';
									}
										...
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
												addHtml += '<div class="first">';
													addHtml += '<div class="hover_cont_one">';
														addHtml += '<img class="attach_img attach_pdf" src="'+crouter_url+'app/webroot/img/file-icon/cleardot.gif">';
													addHtml += '</div>';

													addHtml += '<div class="hover_cont_two">';
														addHtml += '<div class="hover_cont_two_one"><img class="attach_icon" src="'+crouter_url+'app/webroot/img/file-icon/icon_pdf.png"></div>';
														addHtml += '<div class="hover_cont_two_two">'+file.orig_name+'</div>';
													addHtml += '</div>';
													addHtml += '<div class="triangles"><div id="" class="triangles_img triangles_img_pdf" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second second_else">';
													
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_pdf.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size">' + size  + '</div>';
														addHtml += '</div>';
													addHtml += '</div>';
												addHtml += '</div>';
											addHtml += '</a>';
										...
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
												addHtml += '<div class="first">';
													addHtml += '<div class="hover_cont_one">';
														addHtml += '<img class="attach_img attach_text" src="'+crouter_url+'app/webroot/img/file-icon/cleardot.gif">';
													addHtml += '</div>';

													addHtml += '<div class="hover_cont_two">';
														addHtml += '<div class="hover_cont_two_one"><img class="attach_icon" src="'+crouter_url+'app/webroot/img/file-icon/icon_text.png"></div>';
														addHtml += '<div class="hover_cont_two_two">'+file.orig_name+'</div>';
													addHtml += '</div>';
													addHtml += '<div class="triangles "><div id="" class="triangles_img triangles_img_txt" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second second_else">';
													
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_text.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size">' + size  + '</div>';
														addHtml += '</div>';
										...
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
												addHtml += '<div class="first">';
													addHtml += '<div class="hover_cont_one">';
														addHtml += '<img class="attach_img attach_doc" src="'+crouter_url+'app/webroot/img/file-icon/cleardot.gif">';
													addHtml += '</div>';

													addHtml += '<div class="hover_cont_two">';
														addHtml += '<div class="hover_cont_two_one"><img class="attach_icon" src="'+crouter_url+'app/webroot/img/file-icon/icon_doc.png"></div>';
														addHtml += '<div class="hover_cont_two_two">'+file.orig_name+'</div>';
													addHtml += '</div>';
													addHtml += '<div class="triangles "><div id="" class="triangles_img triangles_img_doc" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second second_else">';
													
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_doc.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size">' + size  + '</div>';
														addHtml += '</div>';
										...
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
												addHtml += '<div class="first">';
													addHtml += '<div class="hover_cont_one">';
														addHtml += '<img class="attach_img attach_xls" src="'+crouter_url+'app/webroot/img/file-icon/cleardot.gif">';
													addHtml += '</div>';

													addHtml += '<div class="hover_cont_two">';
														addHtml += '<div class="hover_cont_two_one"><img class="attach_icon" src="'+crouter_url+'app/webroot/img/file-icon/icon_spreadsheet.png"></div>';
														addHtml += '<div class="hover_cont_two_two">'+file.orig_name+'</div>';
													addHtml += '</div>';
													addHtml += '<div class="triangles "><div id="" class="triangles_img triangles_img_xlsx" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second second_else">';
													
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_spreadsheet.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size">' + size  + '</div>';
														addHtml += '</div>';
										...
										addHtml += '<div class="attach_wrap">';
											addHtml += '<a title="'+file.orig_name+'" target="_blank" href="'+file.url+'" data-rel="" class="fancy_'+fancy_count+'">';
												addHtml += '<div class="first">';
													addHtml += '<div class="hover_cont_one">';
														addHtml += '<img class="attach_img attach_zip" src="'+crouter_url+'app/webroot/img/file-icon/cleardot.gif">';
													addHtml += '</div>';

													addHtml += '<div class="hover_cont_two">';
														addHtml += '<div class="hover_cont_two_one"><img class="attach_icon" src="'+crouter_url+'app/webroot/img/file-icon/icon_zip.png"></div>';
														addHtml += '<div class="hover_cont_two_two">'+file.orig_name+'</div>';
													addHtml += '</div>';
													addHtml += '<div class="triangles triangles_img_zip"><div id="" class="triangles_img" style="border-color: #fff"></div></div>';
												addHtml += '</div>';

												addHtml += '<div class="second second_else">';
												
													addHtml += '<div class="hover_cont">';
														addHtml += '<div style="padding: 5px 0 0 5px;min-width: 16px;max-width: 16px;"><img src="'+crouter_url+'app/webroot/img/file-icon/icon_zip.png" alt="icon"></div>';
														addHtml += '<div style="padding-left: 5px;">';
															addHtml += '<div class="attach_name">' + file.orig_name  + '</div>';
															addHtml += '<div class="attach_size">' + size  + '</div>';
														addHtml += '</div>';
									...
									$('.attach').show();
									$('.add_attach').append(addHtml);
									$('.fancy_'+fancy_count).fancybox({
								    buttons : ['fullScreen','close','download'],
								    lang : 'en',
							      i18n : {'en' : {ERROR: err,DOWNLOAD: 'Download'},}
						  		});
								}
		...
		$('.button-again').click(function(){
			$('#form_wizard_1').find('.button-again').hide();
			$('#bar').hide();
			$('#TriggerCustomTextSms').val('').trigger('keyup');
			$('.group_check,#all_group,#all_custom_group,.group_custom_check').removeAttr('checked');
			$.uniform.update()
			customer_custom_groups = [];
			customer_groups = [];
			$('#TriggerCustomSubject').val('');
			$('#custom_text').data("wysihtml5").editor.clear();
			$('#table_file tr.attachbind').html('');
			$('#form_wizard_1').find('.button-next').show();
	       	$('#form_wizard_1').find('.button-previous').hide();
	       	$('#form_wizard_1').find('.button-send').hide();
	       	$('#attach').html('');
	...
	bindQuery:function(){
		mass_communication_list.renderQuery();
		$('#qry_type').select2();
		$('#qry_save').click(function(){
			mass_communication_list.saveQuery();
		});

		var filters = [];
		for(var j in mcl_dt.fields){
			var d = mcl_dt.fields[j].customer_custom_fields;
		...
		for(var j in mcl_dt.getAllCountryList){
			customer_country.push({
				value:j,
				label:mcl_dt.getAllCountryList[j]
			});
		}
		var customer_fields_a = [
			{
				custom_field:'customer_number',
				label:mcl_td.$customerh,
				custom_value:'',
				type:'text',
				custom_type:'integer',
				custom_value:[],
		...
		if(mcl_dt.plan_modules.indexOf('voluntary') != -1){
			var voluntary_fields = [
				{
					...
					custom_type:'string',
					custom_value:[
						{value:'participant',label:mcl_td.Participant},
						{value:'exempt',label:mcl_td.Exempt},
						{value:'specialty_work',label:mcl_td.Specialtywork},
					],
					operators:['equal', 'not_equal'],
					optgroup:mcl_td.Voluntarywork,
					my_type:'voluntary',
					table_name:'customers',
				},
				{
					custom_field:'schedule_date',
					label:mcl_td.ScheduleDate,
					custom_value:'',
					type:'text',
					custom_type:'date',
				...
				},
				{
					...
					label:mcl_td.Schedulestatus,
					custom_value:'',
					type:'select',
					custom_type:'string',
					custom_value:[
						{value:'1',label:mcl_td.Completed},
						{value:'0',label:mcl_td.Inprogress},
					],
					operators:['equal', 'not_equal'],
					optgroup:mcl_td.Voluntarywork,
					my_type:'voluntary',
		...
		if(mcl_dt.plan_modules.indexOf('marina') != -1){
			if(mcl_dt.partner_docks.length != 0){
				var obj = {
					...
					custom_value:[],
					operators:['equal', 'not_equal'],
					optgroup:mcl_td.Marina,
					my_type:'partner_docks',
					table_name:'partner_docks',
				}
				for(var k in mcl_dt.partner_docks){
					var d = mcl_dt.partner_docks[k].partner_docks;
			...
			if(mcl_dt.partner_slips.length != 0){
				var obj = {
				...
				for(var k in mcl_dt.partner_slips){
					var d = mcl_dt.partner_slips[k].partner_slips;
					obj.custom_value.push({
						value:d.slip_number,
						label:d.slip_name
					});
			...
			if(mcl_dt.partner_meters.length != 0){
				var obj = {
				...
				for(var k in mcl_dt.partner_meters){
					var d = mcl_dt.partner_meters[k].partner_meters;
					obj.custom_value.push({
						value:d.id,
						label:d.meter_name
					});
		...
		mass_communication_list.bindFilter(filters);
	},
	renderQuery:function(data=''){
		if(data == ''){
			var data = mcl_dt.mass_comm_query;
		}
		...
		if(data.length == 0){
			...
			mass_communication_list.checkEmpty();
		}
		else{
			for(var j in data){
				var c = data[j].MassCommQuery;
				html += '<tr data-id="'+c.id+'">';
					html += '<td  style="display:none"><input class="uni saveqry" type="checkbox" data-id="'+j+'" onchange="mass_communication_list.setQuery('+j+',4)"></td>';
					html += '<td>'+c.name+'</td>';
					html += '<td>'+c.type+'</td>';
					html += '<td>';
						html += '<button class="sel_qry sel btn mini blue-stripe" href="javascript:;" onclick="mass_communication_list.setQuery('+j+',0)" data-id="'+j+'"><i class="icon-plus"></i> '+mcl_td.Select+'</button>';
						html += '<button style="display:none" class="sel_qry desel btn mini blue-stripe" href="javascript:;" onclick="mass_communication_list.setQuery('+j+',-1)" data-id="'+j+'"><i class="icon-plus"></i> '+mcl_td.Deselect+'</button>';
						html += '&nbsp;<button class="btn mini blue-stripe" href="javascript:;" onclick="mass_communication_list.setQuery('+j+',1)"><i class="icon-eye-open"></i> '+mcl_td.Edit+'</button>&nbsp;';
						html += '<button class="btn mini red-stripe" href="javascript:;" onclick="mass_communication_list.setQuery('+j+',2)"><i class="icon-remove"></i> '+mcl_td.Delete+'</button>';
					html += '</td>';
				html += '</tr>';
			}
			html += '</tbody></table>';
	...
	checkEmpty:function(){
		if($('#query_list tbody tr:not(tr[data-id="0"])').length == 0){
			...
			var html = '<tr><td colspan="3"><div style="margin-bottom: 0;text-align:center;" class="alert alert-error">'+mcl_td.Norecordfound+'</div></td></tr>';
	...
	setQuery:function(j,from){
		...
		if(from == -1){
			...
			mass_communication_list.get_query();
			$('.desel').hide();
			$('.sel').show();
			$('.sel[data-id="'+j+'"]').hide();
			...
			mass_communication_list.setQuery(j,0);
			mass_communication_list.saveandc();
		}
		else if(from == 2){//delete query
			var yes = function(){
				mass_communication_list.deleteQuery(c.id);
			}
			var no = function(){};
			showDeleteMessage(mcl_td.$qry_del,mcl_td.Warning,yes,no,'ui-dialog-blue',mcl_td.Delete,mcl_td.Cancel);
	...
	deleteQuery:function(id){
		...
		params['successCallbackFunction'] = function (complet_data){
			...
			if(complet_data.response.status == 'success'){
				...
				mass_communication_list.checkEmpty();
				$('.buid').hide();
				call_toastr('success',mcl_td.Success, complet_data.response.response.message.msg);
			}
			else if(complet_data.response.status == 'error'){
				if(complet_data.response.response.error == 'validationErrors'){
					var mt_arr = complet_data.response.response.list;
	...
	saveQuery:function(){
		...
		var filter = $('#builder').queryBuilder('getRules', {
	  	...
	  	if(filter.rules.length == 0){
	  		errormsg += mcl_td.Emptyquery+'<br/>';
	  	}
	  	if(checkNull(qry_name) == ''){
	  		errormsg += mcl_td.Name + '<br/>';
	  	}
	  	if(checkNull(qry_type) == ''){
	  		errormsg += mcl_td.Type + '<br/>';
	  	}

	  	if(errormsg != ''){
	  		var msg = mcl_td.Pleasecheckthefollowingfields+'<br/>' + errormsg;
	  		showAlertMessage(msg,'error',mcl_td.alertmessage);
			return;
	  	}
	...
	bindFilter:function(filters){
		...
		QueryBuilder.regional['en'] = {
			...
			"add_rule": mcl_td.$add_rule,
			"add_group": "Legg til gruppe",
			"delete_rule": mcl_td.$delete_rule,
			...
			"conditions": {
		  	...
		  	"operators": {
			  	"equal": mcl_td.$equal,
				"not_equal": mcl_td.$not_equal,
				"in": mcl_td.$in,
				"not_in": mcl_td.$not_in,
				"less": mcl_td.$less,
				"less_or_equal": mcl_td.$less_or_equal,
				"greater": mcl_td.$greater,
				"greater_or_equal": mcl_td.$greater_or_equal,
				"begins_with": mcl_td.$begins_with,
				"not_begins_with": mcl_td.$not_begins_with,
				"contains": mcl_td.$contains,
				"not_contains": mcl_td.$not_contains,
				"ends_with": mcl_td.$ends_with,
				"not_ends_with": mcl_td.$not_ends_with,
				"is_empty": mcl_td.$is_empty,
				"is_not_empty": mcl_td.$is_not_empty,
				"is_null": "er null",
		...
		$('#builder').queryBuilder('destroy');
	 	$('#builder').queryBuilder(options).on('change.rulesChanged', function(e, rule) {
			mass_communication_list.changeBuilder();
	...
	changeBuilder:function(){
			...
			var html = '<button type="button" class="btn btn-xs btn-success" data-add="rule"><i class="icon-plus"></i> '+mcl_td.$add_rule+' </button>';
	...
	create_query:function(){
		...
		mass_communication_list.setQuery(0,-1);
		$('#builder').queryBuilder('reset');
		$('#qry_id').val('');
		$('.query_customer_t').html('');
		customer_queries = [];
	...
	get_query:function(){
		var filter = $('#builder').queryBuilder('getRules', {
     	...
     	mass_communication_list.getCustomers(qry);      	
	},
	getCustomers:function(qry){
		...
		params['successCallbackFunction'] = function (complet_data){
			...
			if(complet_data.response.status == 'success'){
				mass_communication_list.generateCustList(complet_data.response.response.customers);
				mass_communication_list.bindCustList();
			}
			else if(complet_data.response.status == 'error'){
				customer_queries = [];
				mass_communication_list.bindCustList();
		...
		var html = `<table style="margin-top:5px" class="table table-striped table-hover table-bordered">
			<thead>
				<tr>
					...
					<th>`+mcl_td.$customerh+`</th>
					<th>`+mcl_td.Customername+`</th>
				</tr>
			</thead>
			`;
	...
	saveandc:function(){
		...
		$('#saveandcpop').data('modal').options.width = w + 'px';
		$('#qry_name').val('');
		$('#qry_type').val('query').trigger('change');
		$('#qry_id').val('');
		...
		mcl_dt.mass_comm_query
		if(checkNull(c) != '' && checkNull(mcl_dt.mass_comm_query[c] != '')){
			var d = mcl_dt.mass_comm_query[c].MassCommQuery;
	...
	bindCustomGroups:function(){
		...
		$('.group_custom_check').change(function(){
			...
			timeouts.push(
    			setTimeout(function(){
      				mass_communication_list.getCustomGroupData();
    			}, 100)
    		);			
		}).trigger('change');		
	},
	...
	renderSmsSender:function(){
		var sopts = mcl_dt.sender_options;
		var options = '';
		for(var j in sopts){
			if(mcl_dt.getPartnerCustomSettings.sms_sender==j){
				options += '<option value="'+j+'" selected="selected">';
			}
			else{
		...
		$('#TriggerSender').html(options).select2({
	...
	generateCustomerGroupRows:function(data){
		...
		ret += '<tr>';
			...
			ret += '<td>';
				ret += '<a onclick="mass_communication_list.getGroupInCustomer('+partner_customer_groups.id+')" class="btn mini blue-stripe mass_edit mass_edit_'+partner_customer_groups.id+'" style="display:none" data-id="'+partner_customer_groups.id+'"><i class="icon-edit"></i>&nbsp;'+mcl_td.Edit+'</a>';
			ret += '</td>';
		ret += '</tr>';
		return ret;
		//getGroupInCustomer
	...
	getGroupInCustomer:function(group_id){
		...
		params['successCallbackFunction'] = function (complet_data){
			...
			if(complet_data.response.status == 'success'){
				...
				mass_communication_list.generateCustomerList(complet_data.response.response.customersList,group_id);
				//mcl_dt = complet_data.response.response;
	...
	generateCustomerList:function(data,group_id){
		...
		for(var j in data){
			...
			html += '<tr style="cursor:pointer;">';
				...
				html += '<td data-title="'+mcl_td.Customernumber+'">';
					html += cust.customer_number;
				html += '</td>';
				html += '<td data-title="'+mcl_td.Customername+'">';
	...
	showTab:function(which){
		...
		if(which=='tab_1_customers'){
			if(customer_custom_groups.length!=0){
				var msg = mcl_td.$custom_group_err;
				showAlertMessage(msg,'error',mcl_td.alertmessage);
				return;
			}
			...
			if(customer_groups.length!=0){
				var msg = mcl_td.$customer_group_err;
				showAlertMessage(msg,'error',mcl_td.alertmessage);
				return;
			}
	...
	generateCustomerCustomGroupRows:function(data){
		...
		ret += '<tr class="data-row" data-primary-key = "'+PartnerCommunicationGroup.id+'">';
			...
			ret += '<td>';
				ret += '<a onclick="mass_communication_list.editCustomGroup('+PartnerCommunicationGroup.id+')" class="btn mini blue-stripe mass_custom_edit mass_custom_edit_'+PartnerCommunicationGroup.id+'" data-id="'+PartnerCommunicationGroup.id+'"><i class="icon-edit"></i>&nbsp;'+mcl_td.Edit+'</a>';

				ret += '<a onclick="mass_communication_list.deleteCustomGroup('+PartnerCommunicationGroup.id+')" class="btn mini red-stripe mass_custom_del mass_custom_del_'+PartnerCommunicationGroup.id+'" data-id="'+PartnerCommunicationGroup.id+'"><i class="icon-remove"></i>&nbsp;'+mcl_td.Delete+'</a>';
	...
	editCustomGroup:function(id){
		console.log(id);
		new_custom_popup2(800,'popups','add_custom_group',{from:'edit',id:id});
	},
	deleteCustomGroup:function(id){
		var yes = function(){
			...
			params['successCallbackFunction'] = function (complet_data){
				if(complet_data.response.status == 'success'){
					...
					mass_communication_list.getCustomGroupData();
					call_toastr('success',mcl_td.Success, mcl_td.$del_cust_grp_suc);
		...
		showDeleteMessage(mcl_td.$del_cust_grp,mcl_td.Confirmation,yes,no,'ui-dialog-blue',mcl_td.Delete,mcl_td.Cancel);
	},
	generatePlaceholdersList:function(data=''){
		
		if(data==''){
			data = mcl_dt.getPartnerTemplatePlaceholders;
		}
		console.log(data); 
		var ret = '';
		var wsy = '';
		for(var j in data){
			...
			wsy += '<li><a class="cat-header" >' + category + '</a></li>';

			ret += '<optgroup label="'+category+'">';
			for(var k in name){
					ret += '<option value="'+name[k].placeholder+'">'+name[k].value+'</option>';
					wsy += '<li><a data-wysihtml5-command="insertHTML" data-wysihtml5-command-value="'+name[k].placeholder+'">'+name[k].value+'</a></li>';
				}
			ret += '</optgroup>';
		}
		$('.email_placeholders').html(wsy);
		$('#PartnerEmailPlaceholders').html(ret);
	},
	deleteFile:function(url='',that){
		var yes = function(){
			var params = $.extend({}, doAjax_params_default);
			params['url'] = url;
			//wshowProcessingImage('undefined');
			params['requestType'] = 'delete';
			params['completeCallbackFunction'] = function (){
				//hideProcessingImage();
			}
			$(that).parent().remove();
			doAjax(params); return;
		}
		var no = function(){

		}
		showDeleteMessage(mcl_td.$attach_del,mcl_td.Confirmation,yes,no,'ui-dialog-blue',mcl_td.Delete,mcl_td.Cancel);
	},
	sendEmail:function(){
	
		var masscust = $('#masscust').val();
		var TriggerAction = $("#TriggerAction").val();
		var TriggerTo = $('#TriggerTo').val();
		var TriggerSender = $('#TriggerSender').val();
    	...
    	if(TriggerSender=='use_system_phone_number'){
    		var from_sms_no = mcl_dt.use_system_phone_number_v;
    	}
    	else if(TriggerSender=='use_staff_phone_number'){
    		var from_sms_no = mcl_dt.use_staff_phone_number_v;
    	}


    	var params = {};
    	params.from_sms_no = from_sms_no;
    	if(TriggerAction=='send_sms'){
    		params.sms_text = sms;
    	}
    	else if(TriggerAction=='send_email'){
    		params.email_text = email_text;
    		params.email_sub = email_sub;
    	}
    	else if(TriggerAction=='both'){
    		params.sms_text = sms;
    		params.email_text = email_text;
    		params.email_sub = email_sub;
    	}
    	...
    	params.TriggerTo = TriggerTo;
    	params.TriggerSender  = TriggerSender;

    	if(from=='customer_groups'){
    		params.customer_groups = JSON.stringify(customer_groups);
    	}
		else if(from=='custom_groups'){
    		params.customer_custom_groups = JSON.stringify(customer_custom_groups);
    	}
    	else if(from=='queries'){
    		params.queries = JSON.stringify(customer_queries);
    	}
		...
		var total_params =  Object.assign(totalparams, params);


    	var params = $.extend({}, doAjax_params_default);
		params['url'] = APISERVER+'/api/Commons/processMassSmsComm.json';
		params['data'] = total_params;
		params['completeCallbackFunction'] = function (){
			hideProcessingImage();
		}
		...
		params['successCallbackFunction'] = function (complet_data){
			if(complet_data.response.status == 'success'){
				var d = complet_data.response.response;
				mass_communication_list.generateSummary(d);
			}
			else if(complet_data.response.status == 'error'){
				if(complet_data.response.response.error == 'validationErrors'){
					var mt_arr = complet_data.response.response.list;
		...
		showProcessingImage('undefined');
		$('#email_sent,#sms_sent,.generate_summary').parent().addClass('hide');
		doAjax(params);
		return;
	},
	generateSummary:function(data){
		var total_sms_sent = checkNull(data.processMassGroupEmailQueues.total_sms_sent,0);
		total_sms_sent = parseInt(total_sms_sent);

		var total_email_sent = checkNull(data.processMassGroupEmailQueues.total_email_sent,0);
		total_email_sent = parseInt(total_email_sent);

		var total = total_sms_sent + total_email_sent;
		var sms_counter = (checkNull(data.sms_counter)!='')?data.sms_counter:1;

		var a = mcl_td.$recp_count;
		var sms_sent = a.replace('%recp%',total_sms_sent).replace('%counter%',sms_counter);
		...
		$('#customer_reached').html(total).removeClass('hide');
		var v = $('#TriggerAction').val();

		if(v=='send_sms' || v=='both'){
			$('#sms_sent').html(sms_sent).parent().removeClass('hide');
		}
		if(v=='send_email' || v=='both'){
			$('#email_sent').html(total_email_sent).parent().removeClass('hide');
		}
...
Template7.registerHelper('CustomerGroupListHelper', function (data){
	return mass_communication_list.generateCustomerGroupRows(data);
});

Template7.registerHelper('CustomerCustomGroupListHelper', function (data){
	return mass_communication_list.generateCustomerCustomGroupRows(data);
});
