	...
	<div class="span12">	
		<div class="row-fluid" >
			<div class="span12">
				...
				<div id="loadmoreemail" class="row-fluid text-center"><img src="<?php echo Router::url('/',true);  ?>/img/loading.gif"></div>
			</div>
...
function show_msg_body(id,msg_id,sd='',unreadmsgid=''){
		...
		//get message threads start
				...
				var total_params = 'token='+token+'&language='+language+'&partner_id='+partner_id+'&message_id='+msg_id+'&folder='+folders+'&from=inbox&staffer_id='+staffer_id;
...
function delete_letter_email(id,pmid){
		  ...
		  buttons: [
			{
				...
				click: function() {
					...
					var language = '<?php echo CakeSession::read('Config.language'); ?>';
					var lang = '<?php echo CakeSession::read('Config.lang'); ?>';
					var total_params = 'token='+token+'&language='+language+'&lang='+lang+'&id='+id+'&folder=d';
...
function delete_mass_email(){
	 ...
	 var count=0;
		$("input:checkbox:checked").each(function(){
			...
			idArray.push(a[0]);
			count++;
			if(a[1]!=undefined && a[1]!=null && a[1]!='' && a[1]!=0){
				idArray2.push(a[1]);
		  ...
		  buttons: [
			{
				...
				click: function() {
					...
					var language = '<?php echo CakeSession::read('Config.language'); ?>';
					var lang = '<?php echo CakeSession::read('Config.lang'); ?>';
					var total_params = 'token='+token+'&language='+language+'&lang='+lang+'&idArray='+idArray+'&folder=d';
					...
					if(data == undefined){
						...
						if(data.response ){
								...
								for(var k in idArray2){
									var a = parseInt(idArray2[k]);
									$('.ms'+a).remove();
									$('.ms_u'+a).remove();
									$('.ms_m'+a).remove();
									$('.ms_l'+a).remove();
...
function getMoreEmails(text='',from='',config){
	...
	$.ajax({
	    type: 'POST',
	    url: APISERVER+'/api/PartnerMessages/getPartnerLetterDraft.json',
	    data: total_params,
	    async: false,
	    dataType : "json",
	    success: function(data,status,xhr){
...
function appendMoreMailHtml(htmldata,from=''){
	...
	for(var key in htmldata){
		...
		html += '<tr class="ms ms'+PartnerMessageid+cls+' border_box_line test_class" onmouseenter="show_right_menu('+PartnerMessageid+');" onmouseleave="hide_right_menu('+PartnerMessageid+');">';
			html += '<td class="border_box_bottom span1 ms_uid1" onmouseover="show_chk_opt('+PartnerMessageid+');" onmouseout="hide_chk_opt('+PartnerMessageid+');">';
				...
				html += '<div style="text-align:center;">'
							...
							+'<div class="ms_chk_opt ms_chk'+PartnerMessageid+'" style="display: none;">'
								+'<input type="checkbox" class="myCheckBox" value="'+idtodelte+','+pmmiidd+'"/>'
							+'</div>'
						+'</div>';
			html += '</td>';
			var pmidd = htmldata[key].PartnerMessage.parent_message_id;
			var pmtidd = htmldata[key].PartnerMessageText.id;
			if(htmldata[key].hasdraft!=0){
				...
				html += '<td class="border_box_top span10 ms_line'+PartnerMessageid+'" onclick="ss_show_dkmodal_for_editdraft('+pmidd+','+pmtidd+','+PartnerMessageid+');">';
			}
			...
			html += '<td class="border_box span2" style="cursor: default;">';
				html += '<div class="msg_menu_right msg_menu_right'+PartnerMessageid+'" style="text-align: right; margin-right: 20px; display: none; ">';
					...
					var pmmiidd = htmldata[key].PartnerMessage.id;
					var idtodelte = PartnerMessageid;
					if(htmldata[key].hasdraft!=undefined && htmldata.hasdraft!=null && htmldata.hasdraft!=0){
						var idtodelte = htmldata[key].draftid;
					}
					html += '<a class="round_btn_icon btn icn-only" onclick="delete_letter_email('+idtodelte+','+pmmiidd+');" data-toggle="tooltip" data-placement="top" title="Delete" ><i class="icon-trash" style=""></i></a>';
				...
				html += '<div style="" class="reply-msg"><table style="width:100%;"><tbody><tr>';
					html += '<td class="border_box_first" style="border:none; vertical-align: top !important;">';
						if(htmldata[0].PartnerMessage.sender !='customer'){
							var email = '';
							if(htmldata[0].PartnerMessage.source_from!=undefined && htmldata[0].PartnerMessage.source_from!=null && htmldata[0].PartnerMessage.source_from!=''){
								var email = htmldata[0].PartnerMessage.source_from.trim().split(" ");
							}
						}
						else{
							var email = '';
							if(htmldata[0].PartnerMessage.email!=undefined && htmldata[0].PartnerMessage.email!=null && htmldata[0].PartnerMessage.email!=''){
								var email = htmldata[0].PartnerMessage.email.trim().split(" ");
							}
						}
						if(email !=''){
							initial1 =''; initial2 ='';
					...
					html += '<td class="border_box_last"  style="border:none;">';
						html += '<div class="reply-main">';
							html += '<div class="reply-header rpl'+PartnerMessageid+'" style="display: none;">';
								...
								var custname ='';
									if(htmldata[0].Customer.name!= undefined && htmldata[0].Customer.name!=null){

										var custname = htmldata[0].Customer.name;
									}
									var sourceFromEmail = htmldata[0].Customer.name+'</b> < '+custname+' >';
									if(htmldata[0].PartnerMessage.sender !='customer'){
										if(htmldata[0].PartnerMessage.email!='' && htmldata[0].PartnerMessage.email!=undefined && htmldata[0].PartnerMessage.email!=null){
									...
									if(sourceFrom!= undefined && sourceFrom!=null && sourceFrom!=''){
										...
										if(sourceName==''){
											var sourceName = htmldata[0].Customer['name'];
										}
	...
	if(from==''){
		...
		if(html!=''){
			...
			$('#sorting_product_list').after('<div class="alert inboxend alert-info" style="text-align:center;"><?php echo __("Your Draft item listing is empty"); ?></div>');
		}
		...
		$('.searchloader').remove();					
		$('div.alert').remove();
		$('.ms'+PartnerMessageid).remove();
		$('.ms_u'+PartnerMessageid).remove();
		$('.ms_m'+PartnerMessageid).remove();
		$('.ms_l'+PartnerMessageid).remove();
		...
		if(html!=''){
			...
			$('#sorting_product_list').after('<br/><div class="alert inboxend alert-info" style="text-align:center;"><?php echo __("Your Draft item listing is empty"); ?></div>');
		}
		var next_page =  $('#next_page').val();
		$('input[type=checkbox]').uniform();
		//$('body').css('overflow','');
