...
function show_msg_body(id,msg_id,sd='',unreadmsgid=''){
	...
	$('.getmsgthread').remove();
	$('.ms_line'+id+' .writer_name:first').after('<div class="getmsgthread" class="row-fluid text-center"><img src="'+crouter_url+'app/webroot/img/loading.gif"></div>');
	...
	var APISERVER = $('#APISERVER').val();
	//var total_params = 'token='+token+'&language='+language+'&partner_id='+partner_id+'&message_id='+msg_id+'&folder=i&from=inbox';
			...
			if(data.response.status == 'success'){
				...
				if(data.response.response){
					...
					for (index = 0; index < data.response.response.length; ++index){
						text_id =  data.response.response[index].PartnerMessageText.id;
			
						msg_id =  data.response.response[index].PartnerMessage.id;
							...
							if(data.response.response.length > 3){
								...
								if(data.response.response.length > 1){
									...
									if(folder_name!='d'){
										...
										jstext += '<div class="slr_txt_'+id+'_'+msg_id+'" style=" color:#757575; cursor:pointer;">'+$('<div class="testClass">'+text_message+'</div>').text().substr(0,100)+'</div></div></div>';
...
function reply_body_msg_show_tpl(id,parent_message_id,removelocal='yes',message='',attachments=''){
	...
	CKEDITOR.instances['TemplateContent'+id].on('instanceReady', function(){ 
		...
		$('#table-striped_'+id).bind('DOMNodeInserted DOMNodeRemoved',function(){
			$('.draft_progress').show();
			$('.draft_complete').hide();
			$('.sssend,.ssdiscard').attr('disabled','disabled');
			for (var i = 0; i < timeouts.length; i++) {
    			clearTimeout(timeouts[i]);
			}
			timeouts = [];
		  ...
		  buttons: [
			{
				...
				click: function() {
					...
					var data = passProdRequest(APISERVER+'/api/PartnerMessages/deleteLetterMassMail.json',total_params);
					if(data == undefined){
						var json_data = $('#json_data').val();
						var data = JSON.parse(json_data);
						if(data.response ){
					...
					if(data.response.status == 'success'){
						...
						if(draft!='draft'){
							...
							if(data.response.response.partner_message_text_id && data.response.response.partner_message_id && check==null){
								...
								var mark_email_completed= '<?php echo $mark_email_completed ?>';
								if(mark_email_completed=='y'){
									$('.ms'+lineID).remove();
									$('.ms_u'+lineID).remove();
									$('.ms_m'+lineID).remove();
									$('.ms_l'+lineID).remove();
									call_toastr('<?php echo __('success'); ?>', '<?php echo __('Success'); ?>',msg+'<br/><?php echo __("Email marked as completed"); ?>');
								}
								else{
...
function ss_send_msg_fromdraft(parent_message_id){
	...
	var total_params = 'token='+token+'&partner_id='+partner_id+'&partner_message_id='+partner_message_id+'&partner_message_text_id='+partner_message_text_id+'&staffer_id='+staffer_id;
...
function getMoreEmails(text='',from='',config){
	...
	$.ajax({
	    type: 'POST',
	    url: APISERVER+'/api/PartnerMessages/getPartnerLetterInBox.json',
	    data: total_params,
	    async: false,
	    dataType : "json",
	    success: function(data,status,xhr){
			...
			if(data.response.status == 'success'){
				...
				if(data.response.response.pagination.next_page!=null && data.response.response.pagination.next_page!=undefined){
					if(from!='search'){
						...
						if(data.response.response.pagination.next_page!='' && data.response.response.pagination.next_page!=null && data.response.response.pagination.next_page!=undefined){
						...
						if(data.response.response.pagination.start_from!='' && data.response.response.pagination.start_from!=undefined && data.response.response.pagination.start_from!=null){
...
function appendMoreMailHtml(htmldata,from='',add){
	...
	for(var key in htmldata){
		...
		sub = htmldata[key].PartnerMessageText.subject;
		...
		var PartnerMessageFolderId = htmldata[key].PartnerMessageFolder.id;
		/* Initials*/
		...
		html += '<tr class="ms ms'+PartnerMessageid+cls+' border_box_line test_class" onmouseenter="show_right_menu('+PartnerMessageid+');" onmouseleave="hide_right_menu('+PartnerMessageid+');">';
			html += '<td class="border_box_bottom span1 ms_uid1" onmouseover="show_chk_opt('+PartnerMessageid+');" onmouseout="hide_chk_opt('+PartnerMessageid+');">';
				...
				html += '<div style="text-align:center;">'
					html += '<div class="border_radius dp_img ms_img'+PartnerMessageid+'">'+initial1+initial2+'</div>';
					html += '<div class="ms_chk_opt ms_chk'+PartnerMessageid+'" style="display: none;">';
						html += '<input type="checkbox" class="myCheckBox" value="'+PartnerMessageid+'"/>';
					html += '</div>';
				html += '</div>';
			html += '</td>';

			html += '<td class="border_box_top span10 ms_line'+PartnerMessageid+'" onclick="show_msg_body('+PartnerMessageid+','+PartnerMessageid+');">';
				html += '<table><tr class="" style=""><td class="border_box_last" style="padding:0; border:none;" ><div class="span12">';	
					...
					html += '<div class="span3" style="">';
						if(htmldata[key].PartnerMessage.source_from !='' && htmldata[key].PartnerMessage.source_from !=null && htmldata[key].PartnerMessage.source_from != undefined){
							...
							html += '<p id="b"  class="wname writer_name">'+partnername;
							html += '<p class="wname writer_name">,'+cname+'</p>';
							if(htmldata[key].thread != ''){
								html += '<span style="color:#757575">,('+htmldata[key].thread+')</span>';
							}
							html += '</p>';
						}
						else{
							html += '<p id="c" class="wname writer_name">'+partnername
							if(htmldata[key].thread != ''){
								html += '<span style="color:#757575">,('+htmldata[key].thread+')</span>';
							}
							html += '</p>';
			...
			html += '<td class="border_box span3" style="cursor: default;">';
				html += '<div class="msg_menu_right msg_menu_right'+PartnerMessageid+'" style="text-align: right; margin-right: 20px; display: none; ">';
					...
					//html += '<a class="round_btn_icon btn icn-only" onclick="show_dkmodal_for_forward('+PartnerMessageid+','+stext_id+','+smsg_id+',\'all\');" data-toggle="tooltip" data-placement="top" title="<?php echo __("Forward#"); ?>" ><i class="icon-arrow-right" style=""></i></a>';
					...
					html += '<a class="round_btn_icon btn icn-only" onClick="mark_as_compincomp('+PartnerMessageid+','+PartnerMessageFolderId+',\'complete\')" data-toggle="tooltip" data-placement="top" title="<?php echo __("Mark completed"); ?>" ><i class="check-new" style=""></i></a>';
					...
					html += '<div class="btn-group btn-ell pull-right" id="right-btn-ellipsis-'+PartnerMessageid+'" style="">';
						...
						html += '<ul role="menu" class="dropdown-menu pull-right" style="text-align: left;">';
							...
							html += '<li><a class="" onclick="ss_show_dkmodal_for_forward_last('+PartnerMessageid+','+stext_id+','+smsg_id+',\'all\');" data-toggle="tooltip" data-placement="top" title="<?php echo __("Forward#"); ?>" ><i class="icon-arrow-right" style=""></i>&nbsp;<?php echo __("Forward#"); ?></li></a>';
			...
			html += '<div class="main_msg_box allSides" id="show_data">';
				html += '<div class="main_msg_sub">';
					...
					var stext_id = htmldata[key].PartnerMessageText.id;
						...
						html += '<a class="round_btn_icon btn icn-only" onclick="mark_as_compincomp('+PartnerMessageid+','+PartnerMessageFolderId+',\'complete\')" data-toggle="tooltip" data-placement="top" title="<?php echo __("Move to inbox"); ?>" ><i class="check-new" style=""></i></a>';
						...
						html += '<div class="reply-main">';
							html += '<div class="reply-header rpl'+PartnerMessageid+'" style="display: none;">';
								...
								html += '<div class="btn-group">';
									...
									if(htmldata[0].PartnerMessage.sender !='customer'){
										if(htmldata[0].PartnerMessage.source_from!='' && htmldata[0].PartnerMessage.source_from!=undefined && htmldata[0].PartnerMessage.source_from!=null){
											var sourceFrom = htmldata[0].PartnerMessage.email;
										}
									}
									else{
										if(htmldata[0].PartnerMessage.email!='' && htmldata[0].PartnerMessage.email!=undefined && htmldata[0].PartnerMessage.email!=null){
										
											var sourceFrom = htmldata[0].PartnerMessage.source_from;
										}
									}
	...
	if(from==''){
		...
		if(html!=''){
			...
			$('#sorting_product_list').after('<br/><div class="alert inboxend alert-info" style="text-align:center;"><?php echo __("Your inbox item listing is empty"); ?></div>');
			return;
		}
...
$(document).ready(function(){
	...
	$(window).on('scroll', function() {
		if(localStorage.getItem('ajaxstart')=='false' && configured!='no'){
