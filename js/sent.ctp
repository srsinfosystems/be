...
function show_msg_body(id,msg_id,sd='',unreadmsgid=''){
		...
		//get message threads start
				...
				var total_params = 'token='+token+'&language='+language+'&partner_id='+partner_id+'&message_id='+msg_id+'&folder=i&from=inbox'+'&staffer_id='+staffer_id;
		  ...
		  buttons: [
			{
				...
				click: function() {
					...
					var language = '<?php echo CakeSession::read('Config.language'); ?>';
					var lang = '<?php echo CakeSession::read('Config.lang'); ?>';
					var total_params = 'token='+token+'&language='+language+'&lang='+lang+'&idArray='+idArray+'&folder=o';
...
function getMoreEmails(text='',from='',config){
	...
	$.ajax({
	    type: 'POST',
	    url: APISERVER+'/api/PartnerMessages/getPartnerLetterOutBox.json',
	    data: total_params,
	    async: false,
	    dataType : "json",
	    success: function(data,status,xhr){
...
function appendMoreMailHtml(htmldata,from=''){
			...
			html += '<div class="main_msg_box allSides" id="show_data">';
				html += '<div class="main_msg_sub">';
					...
					html += '<span class="span3" style="margin:10px 0px;text-align: right; float:right;">';
						...
						html += '<a class="round_btn_icon btn icn-only" onclick="ss_show_dkmodal_for_forward_last('+PartnerMessageid+');" data-toggle="tooltip" data-placement="top" title="<?php echo __("Forward#"); ?>" ><i class="icon-reply" style=""></i></a>';

						html += '<a class="round_btn_icon btn icn-only" onclick="ss_show_dkmodal_for_reply_last('+PartnerMessageid+');" data-toggle="tooltip" data-placement="top" title="<?php echo __("Reply"); ?>" ><i class="icon-arrow-right" style=""></i></a>';
									...
									html += '<div class="reply-body" style="margin-top:10px;">';
										...
										html += '<div class="reply-body-btn-'+PartnerMessageid+'" style="display:none;">';
											...
											html += '<div>';
												...
												var mpmid = htmldata[0].PartnerMessage.id;
												html += '<button class="btn blue sssend" style="float:left; margin-right:5px;" onclick=\'ss_send_reply_mail('+PartnerMessageid+',\"draft\",\"\",\"yes\");\'><i class="icon-check"></i>&nbsp;<?php echo __("Send"); ?></button>';
												...
												html += '<input id="group_'+PartnerMessageid+'" type="hidden" value="'+htmldata[key].PartnerMessage.group_id+'" name="group_'+PartnerMessageid+'">';
	...
	if(from==''){
		...
		if(html!=''){
			...
			$('#sorting_product_list').after('<div class="alert inboxend alert-info" style="text-align:center;"><?php echo __("Your Sent item listing is empty"); ?></div>');
		}
		...
		if(html!=''){
			...
			$('#sorting_product_list').after('<br/><div class="alert inboxend alert-info" style="text-align:center;"><?php echo __("Your Sent item listing is empty"); ?></div>');
		}
		var next_page =  $('#next_page').val();
		$('input[type=checkbox]').uniform();
		//$('body').css('overflow','');
