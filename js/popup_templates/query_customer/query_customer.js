...
var query_customer = {
	start:function(popupid,metadata={}){
		...
		var total_params = {
			...
			from:'searchget',
			getTranslationsDataArray:['Filter','Cancel','Custom selection','Please correct the errors','$add_rule','$delete_rule','$AND','$OR','$equal','$not_equal','$in','$not_in','$less','$less_or_equal','$greater','$greater_or_equal','$begins_with','$not_begins_with','$contains','$not_contains','$ends_with','$not_ends_with','$is_empty','$is_not_empty'],
		};

		var params = $.extend({}, doAjax_params_default);
		params['url'] = APISERVER+'/api/Customers/getCustomersFilter.json';
		params['data'] = total_params;
		params['successCallbackFunction'] = function (complet_data){
		
			if(complet_data.response.status == 'success'){
				qc_dt = complet_data.response.response;
				qc_td = complet_data.response.response.translationsData;
				query_customer.createHtml(complet_data.response.response);
			}
				...
				if(complet_data.response.response.error == 'validationErrors'){
					...
					showAlertMessage(array,'error',qc_td.Alertmessage);
					return;
				}else{
					showAlertMessage(complet_data.response.response.msg,'error',qc_td.Alertmessage);
					return;
				}	
			}
		}
	...
	createHtml:function(){
		...
		qc_dt.date_format_f = date_format_f;

		var template = document.getElementById('query_customer_template').innerHTML;
		var compiledRendered = Template7(template, qc_dt);
		document.getElementById(qc_popupid).innerHTML = compiledRendered;
		resizemodal(qc_popupid);
		query_customer.bindEvents();
	},
	bindEvents:function(){
		...
		var filters = [];
		for(var j in qc_dt.fields){
			var d = qc_dt.fields[j].customer_custom_fields;
		...
		QueryBuilder.regional['en'] = {
			...
			"add_rule": qc_td.$add_rule,
			"add_group": "Legg til gruppe",
			"delete_rule": qc_td.$delete_rule,
			...
			"conditions": {
		  	...
		  	"operators": {
			  	"equal": qc_td.$equal,
				"not_equal": qc_td.$not_equal,
				"in": qc_td.$in,
				"not_in": qc_td.$not_in,
				"less": qc_td.$less,
				"less_or_equal": qc_td.$less_or_equal,
				"greater": qc_td.$greater,
				"greater_or_equal": qc_td.$greater_or_equal,
				"begins_with": qc_td.$begins_with,
				"not_begins_with": qc_td.$not_begins_with,
				"contains": qc_td.$contains,
				"not_contains": qc_td.$not_contains,
				"ends_with": qc_td.$ends_with,
				"not_ends_with": qc_td.$not_ends_with,
				"is_empty": qc_td.$is_empty,
				"is_not_empty": qc_td.$is_not_empty,
				"is_null": "er null",
		...
		var rules = localStorage.getItem('rules_'+partner_id);
		if(checkNull(rules) != ''){
			$('#builder').queryBuilder('setRules', JSON.parse(rules));
		}

   		$('#query_customer_save').click(function(){
   			query_customer.filterCustomer();
   		});
	},
	filterCustomer:function(){
		try{
      	...
      	if(checkNull(filter) == '' ){
      		showAlertMessage(qc_td.Pleasecorrecttheerrors,'error',qc_td.Alertmessage);
			return;
      	}
      	else if(filter.rules.length == 0){
      		showAlertMessage(qc_td.Pleasecorrecttheerrors,'error',qc_td.Alertmessage);
			return;
      	}
      	else if(checkNull(qry).trim() == ''){
      		showAlertMessage(qc_td.Pleasecorrecttheerrors,'error',qc_td.Alertmessage);
			return;
      	}

      	localStorage.setItem('rules'+partner_id,JSON.stringify($('#builder').queryBuilder('getRules')))
		...
		params['successCallbackFunction'] = function (complet_data){
			...
			if(complet_data.response.status == 'success'){
				if("mass_invoicing" in window){
					$('#'+qc_popupid).modal('hide');
					mass_invoicing.generateExtractCust(complet_data.response.response.fields);
				}
			}
			else if(complet_data.response.status == 'error'){
				if(complet_data.response.response.error == 'validationErrors'){
