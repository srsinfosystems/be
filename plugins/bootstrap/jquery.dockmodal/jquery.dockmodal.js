...
(function ($) {
    var defaults = {
        ...
        opacity: 1,
        dockID : '',
        initialState: 'docked', /* "modal", "docked", "minimized" */
    ...
    var methods = {
        init: function (options) {
            ...
            return this.each(function () {
                ...
                if ($this.options.showClose) {
                    $('<a href="#" style="text-decoration:none;" class="header-action action-close" title="Close"><i class="icon-remove" style="color:#fff;"></i></a>').appendTo($dockHeader).click(function (e) {
                        methods.destroy.apply($this);
                        return false;
                    });
                }
                if ($this.options.showPopout) {
                    $('<a href="#" style="text-decoration:none;" class="header-action action-popout" title="Pop out"><i class="icon-resize-full" style="color:#fff;"></i></a>').appendTo($dockHeader).click(function (e) {
                ...
                if ($this.options.showMinimize) {
                    $('<a href="#" style="text-decoration:none;" class="header-action action-minimize" title="Minimize"><i class="icon-minus" style="color:#fff;"></i></a>').appendTo($dockHeader).click(function (e) {
                ...
                if ($this.options.buttons.length) {
                    ...
                    var $Fcontainer = $('<div></div>').addClass('dkf-container');
                    var $dockFooterButtonset = $('<div style="padding-top: 5px;"></div>').addClass(dClass + "-footer-buttonset pull-left");
                    var $Feditor = $('<div></div>').addClass('dkf-editor pull-left');
						$Feditor.attr("id","dockeditor"+$this.options.dockID);
                    $Fcontainer.append($dockFooterButtonset);
                    ...
                    var $Fattachment = '<span style="font-size: 20px; background-color: inherit; padding: 3px 3px; margin: 5px;"  class="btn pull-left dock-fileinput-btn'+$this.options.dockID+'" data-toggle="tooltip" title="Attachment" ><span style="transform: rotate(45deg); display: block;"><i class="icon-paper-clip"></i></span></span>';
                    //append attachment button befor editor
                    $Fcontainer.append($Fattachment);
                    $Fcontainer.append($Feditor);
                    $Fcontainer.append('<div style="clear:both;"></div>');
                    $dockFooter.append($Fcontainer);
        ...
        refreshLayout: function () {
            ...
            $.each($("." + dClass).toArray().reverse(), function (i, val) {
                ...
                //edited by #080 27-03-2017 save dock state to localStorage  
                var index = $this.options.dockID.substr(5);
				if ($dockModal.hasClass("minimized")) {
                    setdkmodalStorage('modalstate'+index,'minimized');
                }
                else{
					setdkmodalStorage('modalstate'+index,'docked');
				}
